...
export async function getRecipeFromMistral(ingredientsArr) {
  ...
  try {
    ...
    console.log("Raw Mixtral response:", response)

    // Return only the assistant message
    return response.choices?.[0]?.message?.content || "No recipe generated."

  } catch (error) {
    console.error("❌ Error while fetching recipe from Mixtral:", error)
    return "Sorry, there was a problem generating the recipe. Please try again."
  }
}
